<<<<<<< Updated upstream:Assets/Scripts/gen_menu.cs
using UnityEngine;
using UnityEditor;
using System;
...
public class GenerationWindow : EditorWindow
{
    ...
    //private async void Generate() //Ubuntu
...
public class GenerationWindow : EditorWindow
{
    ...
    private string[] premadePrompts = {
        "Select...",
        "Generate world that only has a skybox - nothing else.",
    ...
    private void DrawGenerationsTab()
    {
        ...
        foreach (var kvp in generations)
        {
            ...
            if (editWorld == genName)
            {
                EditorGUI.indentLevel++;
                generationName = genName;
                editPrompt = EditorGUILayout.TextField("Prompt: ", editPrompt);
                
                if (GUILayout.Button("Go", GUILayout.Width(150)))
                {
                    
                    promptText = editPrompt;
                    UnityEngine.Debug.Log($"Prompt:{editPrompt}");
                    Generate();
